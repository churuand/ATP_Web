...
export default function StudentPortal() {
  ...
  const recommendedVideosByCategory = useMemo(() => {
    ...
    allVideos.forEach(video => {
      ...
      const isRecommended = selectedTopics.some(topicId => {
        const topic = mockProblemTopics.find(t => t.id === topicId);
        if (!topic) return false;
        const topicTitle = topic.title.toLowerCase();
        const videoCatName = catName.toLowerCase();
        return videoCatName.includes(topicTitle) || topicTitle.includes(videoCatName);
      });
                                ...
                                <div className="p-4 flex flex-col flex-1">
                                  ...
                                  <div className="flex items-center justify-between mt-auto pt-3 border-t border-gray-50">
                                    <div className="flex items-center gap-1 text-amber-500">
                                      <span className="text-[10px] font-bold">Γÿà</span>
