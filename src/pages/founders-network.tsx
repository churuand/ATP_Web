...
const locationFilters = ["AU", "UK", "US", "CA"] as const;
...
export default function FoundersNetwork() {
  const [selectedGroupId, setSelectedGroupId] = useState<string | null>(afnGroups[0]?.id ?? null);
  const [activeLocation, setActiveLocation] = useState<LocationFilter>(locationFilters[0]);

  const filteredGroups = useMemo(() => afnGroups.filter((group) => group.location === activeLocation), [activeLocation]);
  ...
  const activeGroup = afnGroups.find((group) => group.id === selectedGroupId) ?? filteredGroups[0] ?? null;
        ...
        <div className="container mx-auto px-4">
          <motion.div className="max-w-3xl mx-auto text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            <h1 className="mb-6 text-4xl lg:text-5xl font-serif text-primary">
              AFN <span className="text-accent">Founders Network</span>
            </h1>
      ...
      <section className="bg-white border-b border-gray-100 py-6">
        <div className="container mx-auto px-4">
          <div className="flex flex-col gap-3 rounded-[1.5rem] border border-border bg-muted/50 p-4 md:flex-row md:items-center md:justify-between">
            <p className="text-xs font-semibold uppercase text-gray-600">Sort by location</p>
              ...
              {locationFilters.map((location) => {
                ...
                return (
                  <button
                    type="button"
                    key={location}
                    className={`rounded-full border px-4 py-2 text-[11px] font-semibold uppercase transition ${isActive ? "border-primary bg-primary/10 text-primary" : "border-border bg-card text-gray-600 hover:border-primary"}`}
